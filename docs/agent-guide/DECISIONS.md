...
| ADR-0016 | Provision shared host/container storage with POSIX ACLs | Accepted | 2026-09-04 |
...
- Consequences: Long-running bridge processes hold a shared Linux lifecycle lock and restore requires its exclusive counterpart, reinforcing the documented offline boundary. The production environment file is owned by root, readable only by the `rrpp` group, consumed by the worker service, and loaded into containers by root-run Compose. Deployments remain single-host and are not horizontally scalable. Nginx certificate lifecycle and encrypted off-host transfer remain operator responsibilities. ADR-0016 supersedes the original requirement that the host account and container share UID `10001`; explicit ACLs now preserve shared storage access when those identities differ.
- Validation: Validate unit and Nginx syntax on Ubuntu, confirm only SSH/HTTP/HTTPS listen publicly, exercise the exact webhook route and `404` defaults, run service healthchecks, verify a backup and restore exclusion, and run the automated test suite plus Compose configuration checks.

## ADR-0016: Shared Host and Container Storage ACLs

- Status: Accepted
- Date: 2026-09-04
- Context: The host worker runs as `rrpp`, while hardened containers run as `10001:10001`. Host-owned persistent directories can deny the container access when the host account's numeric identity differs, causing SQLite startup to fail even though both execution identities are correct.
- Decision: Keep both execution identities unchanged. A root-run, idempotent production storage bootstrap creates `var/`, `backups/`, and `backup-export/`; grants explicit POSIX access ACLs to the host worker and container identity on existing directories and files; applies default ACLs to every directory for inheritance; and proves bidirectional creation and append access under a restrictive umask. Every deploy reruns this bootstrap after services stop and before migrations.
- Alternatives: Running containers as root violates least privilege. Recursively changing ownership to only one identity breaks the other execution path. Depending on both identities always having UID `10001` is fragile on existing hosts. A one-time manual ACL command is not repeatable or testable.
- Consequences: Ubuntu production requires the `acl` package plus `runuser` and `setpriv`. Existing host identities remain valid, container hardening is unchanged, and permission drift fails deployment before database migration or service startup. Shared storage remains limited to the three explicit persistent directories.
- Validation: Deployment tests require the bootstrap call before migration and check the storage contract. The bootstrap performs real cross-identity file creation and append probes in all three directories and cleans them up before succeeding.
