...
The supported deployment remains a single Ubuntu host. Hardened containers run web, Instagram ingress, and maintenance with loopback-only published ports; the worker and authenticated OpenClaw Gateway run on the host so their connection remains loopback-only. Host Nginx exposes only the exact Instagram webhook path over HTTPS, while the dashboard remains SSH-tunnel-only. All bridge processes share SQLite on persistent local storage. Runtime processes refuse pending migrations. Service health is durable and the ingress health route is reachable only on the loopback backend, never through Nginx. Backups use SQLite's online backup API, are integrity checked before retention or restore, and encrypted exports use only an owner-controlled public recipient. Restore is never exposed through the dashboard. See ADR-0015.
