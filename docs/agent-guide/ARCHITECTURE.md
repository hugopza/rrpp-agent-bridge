...
| Instagram delivery | `rrpp_bridge.delivery` and `rrpp_bridge.instagram_sender` | Rechecks policy and mode, resolves the sender registered for the durable receiving account, then uses only Meta's official Send API and records the outcome. |
...
| operations and recovery | `rrpp_bridge.operations` and `rrpp_bridge.runtime_lock` | Sanitized service heartbeats, verified backup/retention, retryable `age` export, health evaluation, and lifecycle-locked offline restore. |
...
An RRPP worker obtains a structured decision from its configured provider, then policy decides whether it is eligible. A separate delivery executor rechecks policy, mode, pause state, conversation freshness, idempotency, and the exact receiving-account sender mapping before any official Instagram request. Missing mappings fail closed and never reuse another account's sender. Ambiguous transport outcomes are never retried blindly.
...
The supported deployment remains a single Ubuntu host. Hardened containers run web, Instagram ingress, and maintenance with loopback-only published ports; the worker and authenticated OpenClaw Gateway run on the host so their connection remains loopback-only. Host Nginx exposes only the exact Instagram webhook path over HTTPS, while the dashboard remains SSH-tunnel-only. All bridge processes share SQLite on persistent local storage. An idempotent bootstrap grants access and inherited default ACLs on only `var/`, `backups/`, and `backup-export/` to the host `rrpp` worker and container identity `10001:10001`, then verifies real cross-identity writes before migration. Runtime processes refuse pending migrations and hold a shared Linux lifecycle lock; offline restore requires the corresponding non-blocking exclusive lock. Service health is durable and the ingress health route is reachable only on the loopback backend, never through Nginx. Backups use SQLite's online backup API, are integrity checked before retention or restore, retry failed encrypted exports from the verified local copy, and use only an owner-controlled public recipient. Restore accepts supported older schemas, applies packaged migrations while offline, verifies the result at the current schema, and is never exposed through the dashboard. See ADR-0015 and ADR-0016.
